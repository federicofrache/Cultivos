...
import {
  ...
  Download, RotateCcw, Trash, LayoutDashboard, Menu, Users, ClipboardList, FileDown,
} from "lucide-react";
  ...
  return (
    <div style={{ background: "var(--bg)", fontFamily: "var(--font-body)", color: "var(--ink)", minHeight: "100vh" }}>
      ...
      <main className="max-w-6xl mx-auto px-6 py-6">
        ...
        {nav.view === "ordenes" && (
          <OrdenesTrabajoView
            campanias={campanias} cultivos={cultivos} lotes={lotes} stockInsumos={stockInsumos} insumosCompras={insumosCompras} gastosTodos={gastosTodos}
...
function Breadcrumb({ nav, setNav, campania, cultivo, campo }) {
  if (["campanias", "campos", "insumos", "resumen_general", "papelera", "usuarios", "ordenes", "sociedades"].includes(nav.view)) return null;
  return (
...
function OrdenesTrabajoView({ campanias, cultivos, lotes, stockInsumos, insumosCompras, gastosTodos, ordenes, api, gastosApiGlobal, recalcularConsumos, user, puedeEditar = true }) {
              ...
              <div key={orden.id} className="cc-card p-4">
                <div className="flex items-start justify-between flex-wrap gap-2 mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {puedeEditar && <button onClick={() => eliminarOrden(orden.id)}><Trash2 size={17} color="var(--rust)" /></button>}
                  </div>
                </div>
...
function ResumenGeneralView({ campanias, cultivos, gastos, ventas, remitos, lotes, onOpenCultivo }) {
  ...
  const filasFiltradas = filtroCategoria === "todas" ? filas : filas.filter((f) => f.categoria === filtroCategoria);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => setFiltroCategoria("todas")} className="cc-btn" style={{ padding: "7px 14px", fontSize: 12.5, background: filtroCategoria === "todas" ? "var(--soil)" : "#fff", color: filtroCategoria === "todas" ? "#fff" : "var(--ink)", border: "1px solid var(--line)" }}>Todas</button>
        <button onClick={() => setFiltroCategoria("verano")} className="cc-btn" style={{ padding: "7px 14px", fontSize: 12.5, background: filtroCategoria === "verano" ? "var(--gold)" : "#fff", color: filtroCategoria === "verano" ? "#fff" : "var(--ink)", border: "1px solid var(--line)" }}><Sun size={14} /> Verano</button>
        <button onClick={() => setFiltroCategoria("invierno")} className="cc-btn" style={{ padding: "7px 14px", fontSize: 12.5, background: filtroCategoria === "invierno" ? "var(--frost)" : "#fff", color: filtroCategoria === "invierno" ? "#fff" : "var(--ink)", border: "1px solid var(--line)" }}><Snowflake size={14} /> Invierno</button>
      </div>
