      ...
      <main className="max-w-6xl mx-auto px-6 py-6">
        ...
        {nav.view === "cultivos" && campaniaActual && (
          <CultivosDeCampania campania={campaniaActual} cultivos={cultivos.filter((c) => c.campaniaId === campaniaActual.id)} api={cultivosApi}
            lotes={lotes} remitosTodos={remitosTodos} ventasTodas={ventasTodas} ventasPool={ventasPoolTodas} ventasPoolApi={ventasPoolApi} ventasApiGlobal={ventasApiGlobal}
...
/* ------------------------------------------------------------------ */
function CultivosDeCampania({ campania, cultivos, api, onOpen, puedeEditar = true, lotes = [], remitosTodos = [], ventasTodas = [], ventasPool = [], ventasPoolApi, ventasApiGlobal }) {
  ...
  const grupoCosecha = (() => {
    ...
    const cultivosDelTipo = cultivos.filter((c) => (c.tipo || c.nombre) === tipoActivo);
    if (!cultivosDelTipo.length) return null;
    const catInfo = tiposDisponibles.find((t) => t.tipo === tipoActivo);
    const cultivosConSuperficie = cultivosDelTipo.map((c) => ({
    ...
    const cosechadoKg = remitosTodos.filter((r) => cultivoIds.includes(r.cultivoId)).reduce((s, r) => s + Number(r.kgSL || 0), 0);
