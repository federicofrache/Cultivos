      ...
      <main className="max-w-6xl mx-auto px-6 py-6">
        ...
        {nav.view === "historial" && (
          <HistorialView campos={campos} lotes={lotes} cultivos={cultivos} campanias={campanias} remitosTodos={remitosTodos}
            ventasTodas={ventasTodas} ventasPool={ventasPoolTodas} ventasPoolApi={ventasPoolApi} ventasApiGlobal={ventasApiGlobal} puedeEditar={puedeEditar}
            onOpenCultivo={(cultivoId, campaniaId) => setNav({ view: "cultivo", campaniaId, cultivoId })} />
        )}
...
function HistorialView({ campos, lotes, cultivos, campanias, remitosTodos, ventasTodas, ventasPool, ventasPoolApi, ventasApiGlobal, puedeEditar = true, onOpenCultivo }) {
  const [busqueda, setBusqueda] = useState("");
  ...
  // Agrupar cultivos por tipo (Soja, Maíz...) dentro de cada campaña, para llevar la cosecha y las ventas en conjunto
  const gruposCosecha = {};
  cultivos.forEach((c) => {
    const key = `${c.campaniaId}||${c.tipo || c.nombre}`;
    if (!gruposCosecha[key]) {
      const campania = campanias.find((cp) => cp.id === c.campaniaId);
      gruposCosecha[key] = { campaniaId: c.campaniaId, tipoCultivo: c.tipo || c.nombre, categoria: c.categoria, anio: campania?.anio || 0, campaniaNombre: campania ? (campania.nombre || campania.anio) : "—", cultivos: [] };
    }
    const superficie = lotes.filter((l) => (c.loteIds || []).includes(l.id)).reduce((s, l) => s + Number(l.hectareas || 0), 0);
    gruposCosecha[key].cultivos.push({ id: c.id, nombre: c.nombre, superficie });
  });
  const filasCosecha = Object.values(gruposCosecha).map((g) => {
    const cultivoIds = g.cultivos.map((c) => c.id);
    const superficieTotal = g.cultivos.reduce((s, c) => s + c.superficie, 0);
    ...
    return { ...g, superficieTotal, cosechadoTon, vendidoTon, restanteTon: cosechadoTon - vendidoTon };
  ...
  return (
    <div className="space-y-6">
      ...
      {filasCosecha.length > 0 && (
        <div>
          <div className="cc-h" style={{ fontSize: 17, fontWeight: 600, marginBottom: 4 }}>Cosecha y ventas por cultivo</div>
          <div style={{ fontSize: 12.5, color: "#8A8570", marginBottom: 10 }}>
            Sumás todos los cultivos del mismo tipo (ej. toda la Soja de una campaña), cargás las ventas a medida que las hacés, y cada una se reparte sola entre esos cultivos según su superficie.
          </div>
          <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(340px,1fr))" }}>
            {filasCosecha.map((g) => (
              <CosechaVentaCard key={`${g.campaniaId}||${g.tipoCultivo}`} grupo={g} ventasPool={ventasPool.filter((v) => v.campaniaId === g.campaniaId && v.tipoCultivo === g.tipoCultivo)}
