...
// Normaliza la categoría de un gasto para mostrar siempre "Insumos" (algunos gastos viejos
// quedaron guardados como "Insumo", en singular, antes de unificar el nombre).
const catGasto = (g) => {
  const c = g.categoriaGasto || (g.insumoNombre ? "Insumos" : "Otro");
  return c === "Insumo" ? "Insumos" : c;
};
...
// Kg SL que le corresponden a UN cultivo puntual dentro de un remito puntual (puede ser
// un solo lote, o una porción si el remito es compartido y algunos de esos lotes son de este cultivo).
function kgAtribuidoEnRemito(remito, cultivo, lotesTodos) {
  const ids = remito.loteIds && remito.loteIds.length ? remito.loteIds : (remito.loteId ? [remito.loteId] : []);
  const idsDelCultivo = ids.filter((id) => (cultivo.loteIds || []).includes(id));
  if (!idsDelCultivo.length) return 0;
  if (ids.length <= 1) return Number(remito.kgSL || 0);
  const hectareasTotales = ids.reduce((s, id) => s + Number(lotesTodos.find((l) => l.id === id)?.hectareas || 0), 0);
  if (hectareasTotales > 0) {
    const hectareasDelCultivo = idsDelCultivo.reduce((s, id) => s + Number(lotesTodos.find((l) => l.id === id)?.hectareas || 0), 0);
    return Number(remito.kgSL || 0) * (hectareasDelCultivo / hectareasTotales);
  }
  return Number(remito.kgSL || 0) * (idsDelCultivo.length / ids.length);
}

// Todos los remitos (de una campaña) que tocan a un cultivo puntual, con el kg que le corresponde a cada uno
function remitosDeCultivo(remitosDeLaCampania, cultivo, lotesTodos) {
  return remitosDeLaCampania
    .filter((r) => (r.loteIds || (r.loteId ? [r.loteId] : [])).some((id) => (cultivo.loteIds || []).includes(id)))
    .map((r) => ({ ...r, kgAtribuido: kgAtribuidoEnRemito(r, cultivo, lotesTodos) }));
}
...
export default function App() {
  ...
  const [ajustesStockRaw, setAjustesStockRaw] = useState([]);
  const [ordenesRaw, setOrdenesRaw] = useState([]);
  const [ventasPoolRaw, setVentasPoolRaw] = useState([]);
  ...
  useEffect(() => {
    ...
    const u12 = onSnapshot(collection(db, "ventas_pool"), (s) => setVentasPoolRaw(s.docs.map((d) => ({ id: d.id, ...d.data() }))));
    const u13 = onSnapshot(collection(db, "ajustes_stock"), (s) => setAjustesStockRaw(s.docs.map((d) => ({ id: d.id, ...d.data() }))));
  ...
  const ajustesStock = ajustesStockRaw.filter((x) => !x.eliminado && esDeMiWorkspace(x));
  ...
  const remitosTodos = remitosRaw.filter((x) => !x.eliminado && campaniaIdsActivas.has(x.campaniaId));
  // Remitos cargados antes de este cambio: todavía tienen cultivoId, pero no campaniaId/tipoCultivo
  const remitosSinMigrar = remitosRaw.filter((x) => !x.eliminado && !x.campaniaId && x.cultivoId);
  const ordenesTodas = ordenesRaw.filter((x) => !x.eliminado && cultivoIdsActivos.has(x.cultivoId));
  const ventasPoolTodas = ventasPoolRaw.filter((x) => !x.eliminado && campaniaIdsActivas.has(x.campaniaId));
  ...
  const ajustesStockApi = softDeleteApi("ajustes_stock");
  const ordenesApi = softDeleteApi("ordenes_trabajo");
  const ventasPoolApi = softDeleteApi("ventas_pool");
  ...
  const stockInsumos = (() => {
    ...
    insumosCompras.forEach((c) => {
      ...
      if (!mapa[k]) mapa[k] = { nombre: c.nombre, puntoStockId: c.puntoStockId || null, puntoStockNombre: c.puntoStockNombre || "Sin punto asignado", litrosComprados: 0, costoComprado: 0, litrosConsumidos: 0, ajusteManual: 0, unidad: c.unidad || "Litros" };
    ...
    ajustesStock.forEach((a) => {
      const k = clavePunto(a.nombre, a.puntoStockId);
      if (!mapa[k]) mapa[k] = { nombre: a.nombre, puntoStockId: a.puntoStockId || null, puntoStockNombre: a.puntoStockNombre || "Sin punto asignado", litrosComprados: 0, costoComprado: 0, litrosConsumidos: 0, ajusteManual: 0, unidad: a.unidad || "Litros" };
      mapa[k].ajusteManual += Number(a.litros || 0);
    });
    return Object.values(mapa).map((i) => ({
      ...i,
      disponible: i.litrosComprados - i.litrosConsumidos + i.ajusteManual,
      ...
      <main className="max-w-6xl mx-auto px-6 py-6">
        ...
        {nav.view === "cultivos" && campaniaActual && (
          <CultivosDeCampania campania={campaniaActual} cultivos={cultivos.filter((c) => c.campaniaId === campaniaActual.id)} api={cultivosApi}
            lotes={lotes} campos={campos} remitosTodos={remitosTodos} remitosSinMigrar={remitosSinMigrar} remitosApiGlobal={remitosApiGlobal} ventasTodas={ventasTodas} ventasPool={ventasPoolTodas} ventasPoolApi={ventasPoolApi} ventasApiGlobal={ventasApiGlobal}
            onOpen={(id) => setNav({ view: "cultivo", campaniaId: campaniaActual.id, cultivoId: id })} puedeEditar={puedeEditar} />
        )}

        {nav.view === "cultivo" && cultivoActual && <CultivoDetail cultivo={cultivoActual} lotes={lotes} lotesApi={lotesApi} cultivosApi={cultivosApi} user={user} stockInsumos={stockInsumos} insumosCompras={insumosCompras} gastosTodos={gastosTodos} remitosTodos={remitosTodos} campos={campos} puedeEditar={puedeEditar} miWorkspaceId={miWorkspaceId} />}
        ...
        {nav.view === "insumos" && <InsumosView compras={insumosCompras} api={insumosApi} stockInsumos={stockInsumos} user={user} puedeEditar={puedeEditar} puntosStock={puntosStock} puntosStockApi={puntosStockApi} recalcularConsumos={recalcularConsumosDeInsumo} ajustesStockApi={ajustesStockApi} />}
        ...
        {nav.view === "papelera" && (
          <PapeleraView
            grupos={[
              ...
              { titulo: "Ajustes de stock", items: ajustesStockRaw.filter((x) => x.eliminado && esDeMiWorkspace(x)), api: ajustesStockApi, campo: (x) => `${x.nombre} — ${x.litros >= 0 ? "+" : ""}${fmt(x.litros, 1)} ${abrevUnidad(x.unidad)}` },
              ...
              { titulo: "Ventas por tipo de cultivo", items: ventasPoolRaw.filter((x) => x.eliminado && esDeMiWorkspace(x)), api: ventasPoolApi, campo: (x) => `${x.tipoCultivo} — ${fmt(x.toneladas, 2)} tn` },
            ]}
...
/* ------------------------------------------------------------------ */
function CultivosDeCampania({ campania, cultivos, api, onOpen, puedeEditar = true, lotes = [], campos = [], remitosTodos = [], remitosSinMigrar = [], remitosApiGlobal, ventasTodas = [], ventasPool = [], ventasPoolApi, ventasApiGlobal }) {
  ...
  // Tipos de cultivo presentes en esta campaña (Soja, Maíz, Trigo...), para el filtro de cosecha y ventas
  const tiposDisponibles = Array.from(
    new Map(cultivos.map((c) => [c.tipo || c.nombre, { tipo: c.tipo || c.nombre, categoria: c.categoria }])).values()
  ).sort((a, b) => (a.categoria === b.categoria ? a.tipo.localeCompare(b.tipo) : a.categoria === "verano" ? -1 : 1));
  const [tipoSeleccionado, setTipoSeleccionado] = useState("");
  const tipoActivo = tipoSeleccionado || (tiposDisponibles[0]?.tipo ?? "");

  const cultivosDelTipoActivo = cultivos.filter((c) => (c.tipo || c.nombre) === tipoActivo);
  const idsDelTipoActivo = new Set(cultivosDelTipoActivo.map((c) => c.id));
  const remitosSinMigrarDelTipo = remitosSinMigrar.filter((r) => idsDelTipoActivo.has(r.cultivoId));
  const remitosDelTipoActivo = remitosTodos.filter((r) => r.campaniaId === campania.id && r.tipoCultivo === tipoActivo);

  const grupoCosecha = (() => {
    if (!tipoActivo || !cultivosDelTipoActivo.length) return null;
    const catInfo = tiposDisponibles.find((t) => t.tipo === tipoActivo);
    const cultivosConSuperficie = cultivosDelTipoActivo.map((c) => ({
      id: c.id, nombre: c.nombre,
      superficie: lotes.filter((l) => (c.loteIds || []).includes(l.id)).reduce((s, l) => s + Number(l.hectareas || 0), 0),
    }));
    const cultivoIds = cultivosConSuperficie.map((c) => c.id);
    const superficieTotal = cultivosConSuperficie.reduce((s, c) => s + c.superficie, 0);
    const cosechadoKg = remitosDelTipoActivo.reduce((s, r) => s + Number(r.kgSL || 0), 0);
    const vendidoTon = ventasTodas.filter((v) => cultivoIds.includes(v.cultivoId)).reduce((s, v) => s + Number(v.toneladas || 0), 0);
    const cosechadoTon = cosechadoKg / 1000;
    return {
      campaniaId: campania.id, tipoCultivo: tipoActivo, categoria: catInfo?.categoria || "verano",
      campaniaNombre: campania.nombre || campania.anio, cultivos: cultivosConSuperficie,
      superficieTotal, cosechadoTon, vendidoTon, restanteTon: cosechadoTon - vendidoTon,
    };
  })();
  ...
  return (
    <div className="space-y-5">
      ...
      {tiposDisponibles.length > 0 && (
        <div>
          <div className="cc-h" style={{ fontSize: 16, fontWeight: 600, marginBottom: 4 }}>Cosecha y ventas</div>
          <div style={{ fontSize: 12.5, color: "#8A8570", marginBottom: 8 }}>Elegí un tipo de cultivo para ver cuánto se cosechó, cuánto se vendió, y repartir las ventas entre los cultivos de ese tipo según su superficie.</div>
          <div className="flex flex-wrap gap-2 mb-3">
            {tiposDisponibles.map((t) => (
              <button key={t.tipo} onClick={() => setTipoSeleccionado(t.tipo)} className="cc-btn" style={{ padding: "7px 12px", fontSize: 12.5, background: tipoActivo === t.tipo ? (t.categoria === "verano" ? "var(--gold)" : "var(--frost)") : "#fff", color: tipoActivo === t.tipo ? "#fff" : "var(--ink)", border: "1px solid var(--line)" }}>
                {t.categoria === "verano" ? <Sun size={14} /> : <Snowflake size={14} />} {t.tipo}
              </button>
            ))}
          </div>
          {grupoCosecha && (
            <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(360px,1fr))", alignItems: "start" }}>
              <RemitosCampaniaSection campaniaId={campania.id} tipoCultivo={tipoActivo} categoria={grupoCosecha.categoria} cultivosDelTipo={cultivosDelTipoActivo}
                lotes={lotes} campos={campos} remitos={remitosDelTipoActivo} remitosSinMigrar={remitosSinMigrarDelTipo} remitosApiGlobal={remitosApiGlobal} puedeEditar={puedeEditar} />
              <CosechaVentaCard grupo={grupoCosecha} ventasPool={ventasPool.filter((v) => v.campaniaId === grupoCosecha.campaniaId && v.tipoCultivo === grupoCosecha.tipoCultivo)}
                ventasPoolApi={ventasPoolApi} ventasApiGlobal={ventasApiGlobal} puedeEditar={puedeEditar} />
            </div>
          )}
        </div>
      )}
...
/* ------------------------------------------------------------------ */
function CultivoDetail({ cultivo, lotes, lotesApi, cultivosApi, user, stockInsumos, insumosCompras, gastosTodos, remitosTodos = [], campos, puedeEditar = true, miWorkspaceId }) {
  ...
  // Los remitos ya no se cargan adentro del cultivo: se cargan una sola vez por tipo de
  // cultivo en la pantalla de la Campaña, y acá se calcula qué parte le toca a este cultivo puntual
  // (según de qué lote/es salió cada remito).
  const remitosDeLaCampania = remitosTodos.filter((r) => r.campaniaId === cultivo.campaniaId);
  const remitos = remitosDeCultivo(remitosDeLaCampania, cultivo, lotes);
  ...
  const totalKgSL = remitos.reduce((s, r) => s + Number(r.kgAtribuido || 0), 0);
  ...
  return (
    <div>
      ...
      {tab === "ventas" && <VentasTab cultivo={cultivo} ventas={ventas} />}
      {tab === "remitos" && <RemitosTab cultivo={cultivo} remitos={remitos} totalToneladasVentas={totalToneladasVentas} />}
    </div>
  );
}
...
function GastosTab({ cultivo, gastos, api, user, stockInsumos, insumosCompras, gastosTodos, superficie, puedeEditar = true }) {
  ...
  const editar = (g) => {
    ...
    if (g.insumoNombre) {
      ...
      setForm({ origen: g.origen || "", monto: g.monto ?? "", detalle: g.detalle || "", fecha: g.fecha || "", usuario: g.usuario || user.email, categoriaGasto: "Insumos", socio: g.socio || "" });
    } else {
  ...
  const duplicar = (g) => {
    ...
    if (g.insumoNombre) {
      setTipo("insumo"); setInsumoSel(`${g.insumoNombre}||${g.puntoStockId || "sin_punto"}`); setLitrosUsados(String(g.litrosUsados ?? ""));
      setForm({ origen: g.origen || "", monto: "", detalle: g.detalle || "", fecha: "", usuario: user.email, categoriaGasto: "Insumos", socio: g.socio || "" });
    } else {
      setTipo("general"); setInsumoSel(""); setLitrosUsados("");
  ...
  const guardar = async () => {
    ...
    if (tipo === "insumo") {
      datos = {
        ...
        categoriaGasto: "Insumos", socio: form.socio || "", modoMonto: null, valorPorHectarea: null,
      };
    } else {
    ...
    <div className="space-y-5">
      ...
      {gastos.length > 0 && (
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button className="cc-btn cc-btn-ghost" onClick={() => exportarExcel(`gastos_${cultivo.nombre}`, [{ nombre: "Gastos", filas: gastos.map((g) => ({ Fecha: g.fecha, Categoría: catGasto(g), "N° Orden": g.ordenNumero || "", Origen: g.origen, Socio: g.socio || "", Detalle: g.detalle, Insumo: g.insumoNombre || "", "Litros usados": g.litrosUsados || "", "U$S/ha": g.valorPorHectarea || "", Usuario: g.usuario, Monto: g.monto })) }])}>
              ...
              {[...gastos]
                ...
                <tr key={g.id} style={{ borderTop: "1px solid var(--line)", borderLeft: colorOrden ? `4px solid ${colorOrden}` : "4px solid transparent", background: editId === g.id ? "#FDF3E0" : esProgramado ? "#FDF8EC" : "transparent" }}>
                  ...
                  <td className="px-3 py-2">
                    <span className="cc-chip" style={{ background: (GASTO_CAT_COLOR[catGasto(g)] || GASTO_CAT_COLOR.Otro) + "22", color: GASTO_CAT_COLOR[catGasto(g)] || GASTO_CAT_COLOR.Otro }}>
                      {catGasto(g)}
                    </span>
                  </td>
                  <td className="px-3 py-2">
...
/*  Ingresos · Ventas                                                    */
/* ------------------------------------------------------------------ */
function VentasTab({ cultivo, ventas }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2" style={{ background: "#EEF2F7", border: "1px solid var(--frost)", borderRadius: 8, padding: "10px 12px", fontSize: 13, color: "#2C4A5E" }}>
        <AlertCircle size={16} /> Las ventas ya no se cargan acá — se cargan una sola vez por tipo de cultivo, desde la pantalla de la Campaña ("Cosecha y ventas"), y se reparten solas entre los cultivos según su superficie. Esto es lo que le tocó a este cultivo puntual.
      </div>
      ...
      {ventas.length === 0 ? <EmptyState icon={DollarSign} title="Sin ventas todavía" text="Cuando cargues una venta de este tipo de cultivo en la pantalla de la Campaña, la parte que le corresponde a este cultivo va a aparecer acá." /> : (
        <div className="cc-card overflow-hidden">
          <table className="w-full" style={{ fontSize: 13 }}>
            <thead><tr style={{ background: "#EEEADA", textAlign: "left" }}><th className="px-3 py-2">Fecha</th><th className="px-3 py-2">Origen</th><th className="px-3 py-2 text-right">Toneladas</th><th className="px-3 py-2 text-right">U$S/ton</th><th className="px-3 py-2 text-right">Total</th></tr></thead>
            <tbody>
            ...
            <tfoot><tr style={{ borderTop: "2px solid var(--line)", fontWeight: 700 }}><td className="px-3 py-2" colSpan={2}>Total</td><td className="px-3 py-2 text-right cc-mono">{fmt(totalTon, 2)}</td><td></td><td className="px-3 py-2 text-right cc-mono">{fmtUSD(totalUSD)}</td></tr></tfoot>
          </table>
        </div>
      )}
    </div>
...
/*  Ingresos · Remitos                                                   */
/* ------------------------------------------------------------------ */
function RemitosTab({ cultivo, remitos, totalToneladasVentas }) {
  const totalKgSL = remitos.reduce((s, r) => s + Number(r.kgAtribuido || 0), 0);

  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2" style={{ background: "#EEF2F7", border: "1px solid var(--frost)", borderRadius: 8, padding: "10px 12px", fontSize: 13, color: "#2C4A5E" }}>
        <AlertCircle size={16} /> Los remitos ya no se cargan acá — se cargan una sola vez por tipo de cultivo, desde la pantalla de la Campaña. Acá ves los que le corresponden a este cultivo puntual (según de qué lote salió cada uno).
      </div>

      {remitos.length > 0 && (
        <div className="flex justify-end">
          <button className="cc-btn cc-btn-ghost" onClick={() => exportarExcel(`remitos_${cultivo.nombre}`, [{ nombre: "Remitos", filas: remitos.map((r) => ({ Fecha: r.fecha, Remito: r.remito, Campo: r.campo, Lote: r.lote, Compartido: r.compartido ? "Sí" : "No", Destino: r.destino, "Kg SL total del remito": r.kgSL, "Kg atribuido a este cultivo": Math.round(r.kgAtribuido), "Humedad %": r.humedad })) }])}>
            <Download size={17} /> Exportar Excel
          </button>
        </div>
      )}

      {remitos.length === 0 ? <EmptyState icon={Truck} title="Sin remitos todavía" text="Cuando cargues un remito de este tipo de cultivo (desde la pantalla de la Campaña) que salga de un lote de este cultivo, va a aparecer acá." /> : (
        <div className="cc-card overflow-hidden">
          <table className="w-full" style={{ fontSize: 12.5 }}>
            <thead><tr style={{ background: "#EEEADA", textAlign: "left" }}><th className="px-3 py-2">Fecha</th><th className="px-3 py-2">Remito</th><th className="px-3 py-2">Lote</th><th className="px-3 py-2">Destino</th><th className="px-3 py-2 text-right">Kg SL (de este cultivo)</th><th className="px-3 py-2 text-right">Hum.%</th></tr></thead>
            <tbody>
              {[...remitos].sort((a, b) => (b.fecha || "").localeCompare(a.fecha || "")).map((r) => (
                <tr key={r.id} style={{ borderTop: "1px solid var(--line)" }}>
                  <td className="px-3 py-2 cc-mono">{r.fecha}</td><td className="px-3 py-2">{r.remito}</td>
                  ...
                  <td className="px-3 py-2 text-right cc-mono" style={{ color: !r.kgSL ? "var(--rust)" : "var(--ink)", fontWeight: !r.kgSL ? 700 : 400 }}>{r.kgSL ? fmt(r.kgAtribuido) : "A confirmar"}</td>
                  <td className="px-3 py-2 text-right cc-mono">{fmt(r.humedad, 1)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot><tr style={{ borderTop: "2px solid var(--line)", fontWeight: 700 }}><td className="px-3 py-2" colSpan={4}>Total Kg SL</td><td className="px-3 py-2 text-right cc-mono">{fmt(totalKgSL)}</td><td></td></tr></tfoot>
          </table>
...
function InsumosView({ compras, api, stockInsumos, user, puedeEditar = true, puntosStock, puntosStockApi, recalcularConsumos, ajustesStockApi }) {
  ...
  const [editandoStockKey, setEditandoStockKey] = useState(null);
  const [valorStockEditado, setValorStockEditado] = useState("");
  ...
  const empezarAjusteStock = (i) => {
    setEditandoStockKey(`${i.nombre}||${i.puntoStockId || "sin_punto"}`);
    setValorStockEditado(String(i.disponible));
  };
  const guardarAjusteStock = async (i) => {
    const nuevoValor = Number(valorStockEditado);
    if (isNaN(nuevoValor)) { alert("Ingresá un número válido."); return; }
    const diferencia = nuevoValor - i.disponible;
    if (Math.abs(diferencia) < 0.001) { setEditandoStockKey(null); return; }
    setAjustando(true);
    try {
      await ajustesStockApi.add({
        nombre: i.nombre, puntoStockId: i.puntoStockId || null, puntoStockNombre: i.puntoStockNombre, unidad: i.unidad,
        litros: diferencia, motivo: "Ajuste manual de stock", fecha: new Date().toISOString().slice(0, 10),
      });
      setEditandoStockKey(null);
    } finally { setAjustando(false); }
  };

  const puntosPresentes = Array.from(new Set(stockInsumos.map((i) => i.puntoStockNombre)));
  const stockFiltrado = stockInsumos.filter((i) => {
    if (!mostrarAgotados && i.disponible === 0) return false;
    if (filtroInsumo.trim() && !i.nombre.toLowerCase().includes(filtroInsumo.trim().toLowerCase())) return false;
    if (filtroPunto !== "todos" && i.puntoStockNombre !== filtroPunto) return false;
    return true;
  });
        ...
        {stockInsumos.some((i) => i.disponible < 0) && (
          <div className="flex items-start gap-2 mb-3" style={{ background: "#FBEAEA", border: "1px solid var(--rust)", borderRadius: 8, padding: "10px 12px" }}>
            ...
            <div style={{ fontSize: 13, color: "#7A2E2E" }}>
              <b>Stock negativo:</b> {stockInsumos.filter((i) => i.disponible < 0).map((i) => i.nombre).join(", ")}. Revisá si falta cargar alguna compra, o ajustá el stock a mano.
            </div>
          </div>
        )}
          ...
          <>
            <div className="flex gap-3 flex-wrap items-end mb-3">
              <div style={{ minWidth: 180, flex: 1 }}>
                ...
                <input className="cc-input" value={filtroInsumo} onChange={(e) => setFiltroInsumo(e.target.value)} placeholder="Buscar insumo..." />
              </div>
              <div style={{ minWidth: 180 }}>
                <label style={{ fontSize: 12, color: "#8A8570" }}>Punto de stock</label>
                <select className="cc-input" value={filtroPunto} onChange={(e) => setFiltroPunto(e.target.value)}>
                  <option value="todos">Todos</option>
                  {puntosPresentes.map((p) => <option key={p} value={p}>{p}</option>)}
                </select>
              </div>
              <label className="flex items-center gap-2" style={{ fontSize: 12.5, color: "#5A5647", paddingBottom: 10, cursor: "pointer" }}>
                <input type="checkbox" checked={mostrarAgotados} onChange={(e) => setMostrarAgotados(e.target.checked)} />
                Mostrar agotados (en 0)
              </label>
            </div>

            {stockFiltrado.length === 0 ? (
              <div style={{ fontSize: 13, color: "#8A8570" }}>No hay insumos que coincidan con el filtro.</div>
            ) : (
              <div className="cc-card overflow-hidden">
                <table className="w-full" style={{ fontSize: 13 }}>
                  <thead><tr style={{ background: "#EEEADA", textAlign: "left" }}><th className="px-3 py-2">Insumo</th><th className="px-3 py-2">Punto de stock</th><th className="px-3 py-2 text-right">Comprado</th><th className="px-3 py-2 text-right">Consumido</th><th className="px-3 py-2 text-right">Disponible</th><th className="px-3 py-2 text-right">Costo prom. / unidad</th><th className="px-3 py-2"></th></tr></thead>
                  <tbody>
                    {[...stockFiltrado].sort((a, b) => a.nombre.localeCompare(b.nombre) || a.puntoStockNombre.localeCompare(b.puntoStockNombre)).map((i) => {
                      const key = `${i.nombre}||${i.puntoStockId || "sin_punto"}`;
                      const editando = editandoStockKey === key;
                      return (
                        <tr key={key} style={{ borderTop: "1px solid var(--line)", background: i.disponible < 0 ? "#FBEAEA" : editando ? "#FDF3E0" : "transparent" }}>
                          ...
                          <td className="px-3 py-2 text-right cc-mono" style={{ color: i.disponible < 0 ? "var(--rust)" : i.disponible === 0 ? "#8A8570" : "var(--soil-light)", fontWeight: 700 }}>
                            {editando ? (
                              <input className="cc-input" style={{ padding: "4px 8px", fontSize: 12.5, width: 100, display: "inline-block" }} type="number" value={valorStockEditado} onChange={(e) => setValorStockEditado(e.target.value)} autoFocus />
                            ) : (
                              <>{fmt(i.disponible, 1)} {abrevUnidad(i.unidad)}</>
                            )}
                          </td>
                            ...
                            {puedeEditar && (
                              editando ? (
                                <div className="flex gap-1 justify-end">
                                  <button onClick={() => guardarAjusteStock(i)} disabled={ajustando} title="Guardar">{ajustando ? <Loader2 size={15} className="animate-spin" /> : <CheckCircle2 size={15} color="var(--soil-light)" />}</button>
                                  <button onClick={() => setEditandoStockKey(null)} title="Cancelar"><X size={15} color="var(--rust)" /></button>
                                </div>
                              ) : (
                                <button onClick={() => empezarAjusteStock(i)} title="Ajustar stock manualmente"><Pencil size={15} color="var(--frost)" /></button>
                              )
                            )}
                          </td>
                        </tr>
                ...
                <div className="px-3 py-2" style={{ fontSize: 11.5, color: "#8A8570", borderTop: "1px solid var(--line)" }}>
                  El lápiz te deja corregir el disponible a mano (por ejemplo, después de un recuento físico). El ajuste queda registrado aparte, sin cambiar el costo promedio calculado a partir de las compras.
                </div>
              </div>
            )}
          </>
  ...
  const confirmarLitros = async (orden, idx, valorCrudo) => {
    ...
    try {
      ...
      const nuevoGasto = await gastosApiGlobal.add({
        ...
        categoriaGasto: "Insumos", socio: orden.socio || "", modoMonto: null, valorPorHectarea: null, ordenTrabajoId: orden.id, ordenNumero: orden.numero || null,
      });
      await recalcularConsumos(item.insumoNombre, item.puntoStockId || null);
      const nuevosItems = orden.items.map((it, i) => (i === idx ? { ...it, litrosReales: litros, confirmado: true, gastoId: nuevoGasto.id, gastoIds: [nuevoGasto.id], fuentes: [{ puntoStockId: item.puntoStockId || null, puntoStockNombre: item.puntoStockNombre || "", litros }] } : it));
      await api.update(orden.id, { items: nuevosItems });
    } finally { setConfirmandoId(null); }
  };

  // Igual que confirmarLitros, pero repartiendo el consumo entre varios puntos de stock del
  // mismo insumo (ej: compraste el mismo producto en dos lugares distintos y usaste de los dos)
  const confirmarLitrosSplit = async (orden, idx, splits) => {
    const splitsValidos = splits.filter((s) => s.puntoStockKey && Number(s.litros) > 0);
    if (!splitsValidos.length) { alert("Completá al menos un punto de stock con una cantidad mayor a 0."); return; }
    const item = orden.items[idx];
    setConfirmandoId(`${orden.id}-${idx}`);
    try {
      const fuentes = [];
      for (const s of splitsValidos) {
        const puntoStockId = s.puntoStockKey === "sin_punto" ? null : s.puntoStockKey;
        const comprasRelevantes = insumosCompras.filter((c) => c.nombre === item.insumoNombre && (c.puntoStockId || null) === puntoStockId);
        const yaConsumido = gastosTodos
          .filter((g) => g.insumoNombre === item.insumoNombre && (g.puntoStockId || null) === puntoStockId)
          .reduce((sum, g) => sum + Number(g.litrosUsados || 0), 0);
        const fifo = costoFIFO(comprasRelevantes, yaConsumido, Number(s.litros));
        const nuevoGasto = await gastosApiGlobal.add({
          cultivoId: orden.cultivoId, origen: item.insumoNombre,
          monto: fifo.costoTotal, detalle: orden.detalle ? `${orden.detalle} — ${item.insumoNombre} (${s.puntoStockNombre})` : `Consumo de ${item.insumoNombre} (${s.puntoStockNombre})`,
          fecha: orden.fecha, usuario: user.email, insumoNombre: item.insumoNombre, litrosUsados: Number(s.litros),
          costoPorLitro: fifo.costoPromedioEfectivo, puntoStockId, puntoStockNombre: s.puntoStockNombre || "",
          categoriaGasto: "Insumos", socio: orden.socio || "", modoMonto: null, valorPorHectarea: null, ordenTrabajoId: orden.id, ordenNumero: orden.numero || null,
        });
        await recalcularConsumos(item.insumoNombre, puntoStockId);
        fuentes.push({ gastoId: nuevoGasto.id, puntoStockId, puntoStockNombre: s.puntoStockNombre, litros: Number(s.litros) });
      }
      const totalLitros = fuentes.reduce((s, f) => s + f.litros, 0);
      const nuevosItems = orden.items.map((it, i) => (i === idx ? {
        ...it, litrosReales: totalLitros, confirmado: true, gastoId: fuentes[0]?.gastoId || null, gastoIds: fuentes.map((f) => f.gastoId), fuentes,
      } : it));
      await api.update(orden.id, { items: nuevosItems });
    } finally { setConfirmandoId(null); }
  };
      ...
      {ordenes.length === 0 ? (
        ...
        <div className="space-y-4">
          {[...ordenes].sort((a, b) => (Number(b.numero) || 0) - (Number(a.numero) || 0)).map((orden) => {
            ...
            const colorOrden = orden.numero ? colorPorTexto(`orden-${orden.numero}`) : "#2C3A24";
            return (
              <div key={orden.id} className="cc-card p-4" style={{ borderLeft: `5px solid ${colorOrden}` }}>
                <div className="flex items-start justify-between flex-wrap gap-2 mb-3">
                  <div>
                    <div className="cc-h" style={{ fontSize: 16, fontWeight: 600 }}>
                      {orden.numero && <span className="cc-chip" style={{ background: colorOrden, color: "#fff", marginRight: 8 }}>N° {orden.numero}</span>}
                  ...
                  <table className="w-full" style={{ fontSize: 12.5 }}>
                    ...
                    <tbody>
                      {(orden.items || []).map((it, idx) => (
                        <ItemOrdenFila key={idx} item={it} onConfirmar={(valor) => confirmarLitros(orden, idx, valor)} onConfirmarSplit={(splits) => confirmarLitrosSplit(orden, idx, splits)} onAsociar={(key) => asociarItemAStock(orden, idx, key)} stockInsumos={stockInsumos} procesando={confirmandoId === `${orden.id}-${idx}`} puedeEditar={puedeEditar} />
                      ))}
                    </tbody>
                  </table>
                </div>
...
function ItemOrdenFila({ item, onConfirmar, onConfirmarSplit, onAsociar, stockInsumos, procesando, puedeEditar }) {
  ...
  const [splits, setSplits] = useState([{ puntoStockKey: "", puntoStockNombre: "", litros: "" }]);
  ...
  // Otros puntos de stock donde también hay (o hubo) este mismo insumo — para poder repartir el consumo
  const opcionesPunto = stockInsumos.filter((s) => s.nombre === item.insumoNombre);
  const setSplit = (i, next) => setSplits(splits.map((s, idx) => (idx === i ? next : s)));
  const agregarSplit = () => setSplits([...splits, { puntoStockKey: "", puntoStockNombre: "", litros: "" }]);
  const quitarSplit = (i) => setSplits(splits.filter((_, idx) => idx !== i));
  const totalSplit = splits.reduce((s, x) => s + (Number(x.litros) || 0), 0);
  const guardarSplit = () => {
    const splitsConNombre = splits.map((s) => {
      const opt = opcionesPunto.find((o) => `${o.puntoStockId || "sin_punto"}` === s.puntoStockKey);
      return { ...s, puntoStockNombre: opt?.puntoStockNombre || "Sin punto asignado" };
    });
    onConfirmarSplit(splitsConNombre);
  };
  ...
  return (
    <>
      <tr style={{ borderTop: "1px solid var(--line)" }}>
        ...
        <td className="px-3 py-2" style={{ color: "#5A5647" }}>{item.confirmado && item.fuentes && item.fuentes.length > 1 ? "Varios puntos" : item.puntoStockNombre}</td>
        <td className="px-3 py-2 text-right cc-mono">{fmt(item.dosisPorHa, 2)} {abrevUnidad(item.unidad)}</td>
        <td className="px-3 py-2 text-right cc-mono">{fmt(item.litrosCalculados, 1)} {abrevUnidad(item.unidad)}</td>
        <td className="px-3 py-2 text-right">
          {item.confirmado ? (
            ...
            <span style={{ fontSize: 11.5, color: "#8A8570" }}>Ver abajo</span>
          )}
        </td>
        <td className="px-3 py-2 text-right">
          {!item.confirmado && puedeEditar && !modoSplit && (
      ...
      {!item.confirmado && puedeEditar && !modoSplit && opcionesPunto.length > 1 && (
        <tr>
          <td colSpan={6} className="px-3" style={{ paddingBottom: 8, paddingTop: 0 }}>
            <button onClick={() => setModoSplit(true)} style={{ fontSize: 11.5, color: "var(--frost)" }}>¿Lo usaste de más de un punto de stock?</button>
          </td>
        </tr>
      )}
      {!item.confirmado && puedeEditar && modoSplit && (
        <tr>
          <td colSpan={6} className="px-3 py-2" style={{ background: "#F7F5EC" }}>
            <div style={{ fontSize: 11.5, color: "#8A8570", marginBottom: 6 }}>Repartí los {fmt(item.litrosCalculados, 1)} {abrevUnidad(item.unidad)} necesarios entre los puntos de stock que usaste:</div>
            <div className="space-y-2">
              {splits.map((s, i) => (
                <div key={i} className="flex gap-2 items-center flex-wrap">
                  <select className="cc-input" style={{ padding: "5px 8px", fontSize: 12, width: 220 }} value={s.puntoStockKey} onChange={(e) => setSplit(i, { ...s, puntoStockKey: e.target.value })}>
                    <option value="">Elegir punto de stock...</option>
                    {opcionesPunto.map((o) => (
                      <option key={o.puntoStockId || "sin_punto"} value={o.puntoStockId || "sin_punto"}>
                        {o.puntoStockNombre} ({fmt(o.disponible, 1)} {abrevUnidad(o.unidad)} disp.)
                      </option>
                    ))}
                  </select>
                  <input className="cc-input" style={{ padding: "5px 8px", fontSize: 12, width: 100 }} type="number" value={s.litros} onChange={(e) => setSplit(i, { ...s, litros: e.target.value })} placeholder="Litros" />
                  {splits.length > 1 && <button onClick={() => quitarSplit(i)}><X size={14} color="var(--rust)" /></button>}
                </div>
              ))}
            </div>
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              <button className="cc-btn cc-btn-ghost" style={{ padding: "5px 10px", fontSize: 11.5 }} onClick={agregarSplit}><Plus size={13} /> Agregar otro punto</button>
              <span style={{ fontSize: 12, color: "#8A8570" }}>Total: <b style={{ color: "var(--ink)" }}>{fmt(totalSplit, 1)} {abrevUnidad(item.unidad)}</b></span>
              <button className="cc-btn cc-btn-primary" style={{ padding: "5px 10px", fontSize: 12 }} onClick={guardarSplit} disabled={procesando}>
                {procesando ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />} Guardar reparto
              </button>
              <button className="cc-btn cc-btn-ghost" style={{ padding: "5px 10px", fontSize: 12 }} onClick={() => setModoSplit(false)}>Cancelar</button>
            </div>
          </td>
        </tr>
      )}
      {item.confirmado && item.fuentes && item.fuentes.length > 1 && (
        <tr>
          <td colSpan={6} className="px-3" style={{ paddingBottom: 8, paddingTop: 0, fontSize: 11, color: "#8A8570" }}>
            Repartido: {item.fuentes.map((f) => `${fmt(f.litros, 1)} ${abrevUnidad(item.unidad)} de ${f.puntoStockNombre}`).join(" + ")}
          </td>
        </tr>
      )}
    </>
...
function ResumenGeneralView({ campanias, cultivos, gastos, ventas, remitos, lotes, onOpenCultivo }) {
  ...
  const filas = cultivos.map((c) => {
    ...
    const remitosDeLaCampania = remitos.filter((r) => r.campaniaId === c.campaniaId);
    ...
    const totalTon = (c.loteIds || []).reduce((s, loteId) => s + kgAtribuidoALote(remitosDeLaCampania, loteId, lotes), 0) / 1000;
  ...
  const rentabilidadGeneral = totalGastosGeneral ? (margenGeneral / totalGastosGeneral) * 100 : null;

  // Desglose de gastos por tipo (Insumo, Servicio, Renta...), solo de los cultivos que están en el filtro actual
  const cultivoIdsFiltrados = new Set(filasFiltradas.map((f) => f.cultivoId));
  const gastosFiltrados = gastos.filter((g) => cultivoIdsFiltrados.has(g.cultivoId));
  const gastosPorTipo = {};
  gastosFiltrados.forEach((g) => {
    const cat = catGasto(g);
    gastosPorTipo[cat] = (gastosPorTipo[cat] || 0) + Number(g.monto || 0);
  });
  const filasGastosPorTipo = Object.entries(gastosPorTipo).sort((a, b) => b[1] - a[1]);
  ...
  const exportar = () => exportarExcel("resumen_general_campo_costo", [
    ...
    { nombre: "Gastos por tipo", filas: [...filasGastosPorTipo.map(([cat, monto]) => ({ Tipo: cat, Monto: monto, "% del total": totalGastosGeneral ? (monto / totalGastosGeneral) * 100 : 0 })), { Tipo: "Rentabilidad (%)", Monto: rentabilidadGeneral, "% del total": "" }] },
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(200px,1fr))" }}>
        ...
        <StatCard label="Rentabilidad" value={rentabilidadGeneral !== null ? `${rentabilidadGeneral >= 0 ? "+" : ""}${fmt(rentabilidadGeneral, 1)}%` : "-"} icon={rentabilidadGeneral >= 0 ? TrendingUp : TrendingDown} color={rentabilidadGeneral >= 0 ? "var(--soil-light)" : "var(--rust)"} />
      </div>
      <div style={{ fontSize: 11.5, color: "#8A8570", marginTop: -12 }}>La rentabilidad es el margen como porcentaje de lo gastado (cuánto ganaste por cada dólar invertido).</div>
      ...
      {filasGastosPorTipo.length > 0 && (
        <div className="cc-card p-4">
          <div className="cc-h" style={{ fontSize: 15, fontWeight: 600, marginBottom: 10 }}>Gastos por tipo</div>
          <div className="space-y-3">
            {filasGastosPorTipo.map(([cat, monto]) => {
              const pct = totalGastosGeneral ? (monto / totalGastosGeneral) * 100 : 0;
              const color = GASTO_CAT_COLOR[cat] || GASTO_CAT_COLOR.Otro;
              return (
                <div key={cat}>
                  <div className="flex items-center justify-between" style={{ fontSize: 13 }}>
...
/* ------------------------------------------------------------------ */
/* ------------------------------------------------------------------ */
/*  Cosecha y ventas de un tipo de cultivo (pool compartido entre       */
/*  todos los cultivos de ese tipo en la misma campaña)                 */
/* ------------------------------------------------------------------ */
/* ------------------------------------------------------------------ */
/*  Remitos de un tipo de cultivo dentro de una campaña (único lugar    */
/*  para cargarlos — ya no se cargan adentro de cada cultivo puntual)   */
/* ------------------------------------------------------------------ */
function RemitosCampaniaSection({ campaniaId, tipoCultivo, categoria, cultivosDelTipo, lotes, campos, remitos, remitosSinMigrar = [], remitosApiGlobal, puedeEditar = true }) {
  const [migrando, setMigrando] = useState(false);

  const migrarRemitosViejos = async () => {
    if (!confirm(`Se va a completar el campaña/tipo de cultivo de ${remitosSinMigrar.length} remito(s) viejo(s), usando el cultivo al que estaban asociados antes. ¿Continuar?`)) return;
    setMigrando(true);
    try {
      await Promise.all(remitosSinMigrar.map((r) => {
        const loteIds = r.loteIds && r.loteIds.length ? r.loteIds : (r.loteId ? [r.loteId] : []);
        return remitosApiGlobal.update(r.id, { campaniaId, tipoCultivo, categoria, loteIds });
      }));
  ...
  const loteIdsDelTipo = new Set(cultivosDelTipo.flatMap((c) => c.loteIds || []));
  const lotesDelTipo = lotes.filter((l) => loteIdsDelTipo.has(l.id));
  const camposConLotes = campos.filter((c) => lotesDelTipo.some((l) => l.campoId === c.id));
  const lotesDelCampo = form.campoId ? lotesDelTipo.filter((l) => l.campoId === form.campoId) : [];
  const gruposCampoLote = [...camposConLotes.map((c) => c.id), "__sin_campo__"];
  ...
  const guardar = () => {
    ...
    if (!compartido && !form.loteId) { alert("Elegí de qué lote sale este remito."); return; }
    ...
    const datos = {
      ...
      loteIds, compartido, campaniaId, tipoCultivo, categoria,
      destino: form.destino,
      kgTolva: Number(form.kgTolva || 0), kgBrutos: Number(form.kgBrutos || 0), kgSL: Number(form.kgSL || 0), humedad: Number(form.humedad || 0),
    };

    if (editId) { remitosApiGlobal.update(editId, datos); } else { remitosApiGlobal.add(datos); }
    limpiarForm(); setMostrandoForm(false);
    setMensaje(editId ? "Remito actualizado ✓" : "Remito guardado ✓"); setTimeout(() => setMensaje(""), 2500);
  };
  const eliminar = (id) => { if (confirm("Este remito se moverá a la papelera. ¿Continuar?")) remitosApiGlobal.remove(id); };
  ...
  return (
    <div className="cc-card p-4">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div className="cc-h" style={{ fontSize: 15, fontWeight: 600 }}>Remitos</div>
        <span className="cc-mono" style={{ fontSize: 13, color: "#8A8570" }}>{fmt(totalKgSL, 0)} kg totales</span>
      </div>

      {puedeEditar && remitosSinMigrar.length > 0 && (
        <div className="flex items-center justify-between flex-wrap gap-2 mb-3" style={{ background: "#FDF3E0", border: "1px solid var(--gold)", borderRadius: 8, padding: "8px 12px" }}>
          <span style={{ fontSize: 12.5, color: "#7A5A1E" }}>Tenés {remitosSinMigrar.length} remito(s) de "{tipoCultivo}" cargado(s) antes de esta actualización, sin migrar todavía.</span>
          <button className="cc-btn" style={{ padding: "6px 10px", fontSize: 12, background: "var(--gold)", color: "#fff" }} onClick={migrarRemitosViejos} disabled={migrando}>
            {migrando ? <Loader2 size={14} className="animate-spin" /> : <RotateCcw size={14} />} Migrarlos acá
          </button>
        </div>
      )}

      {lotesDelTipo.length === 0 ? (
        <div style={{ fontSize: 12.5, color: "#8A8570" }}>Ninguno de los cultivos de "{tipoCultivo}" tiene lotes asignados todavía — andá a la pestaña "Lotes" de cada cultivo primero.</div>
      ) : (
        <>
          {puedeEditar && !mostrandoForm && <button className="cc-btn cc-btn-ghost mb-3" onClick={() => setMostrandoForm(true)}><Plus size={16} /> Cargar remito</button>}

          {puedeEditar && mostrandoForm && (
            <div className="p-3 mb-3 space-y-3" style={{ background: "#F7F5EC", borderRadius: 8, border: "1px solid var(--line)" }}>
              {editId && (
                <div className="flex items-center gap-2" style={{ background: "#FDF3E0", border: "1px solid var(--gold)", borderRadius: 8, padding: "6px 10px", fontSize: 12, color: "#7A5A1E" }}>
                  <Pencil size={14} /> Editando un remito ya guardado.
                </div>
              )}
              <div className="flex gap-1">
                <button type="button" onClick={() => { setCompartido(false); setLoteIdsCompartido([]); }} className="cc-btn" style={{ padding: "6px 10px", fontSize: 12, background: !compartido ? "var(--soil)" : "#fff", color: !compartido ? "#fff" : "var(--ink)", border: "1px solid var(--line)" }}>Un solo lote</button>
                <button type="button" onClick={() => setCompartido(true)} className="cc-btn" style={{ padding: "6px 10px", fontSize: 12, background: compartido ? "var(--gold)" : "#fff", color: compartido ? "#fff" : "var(--ink)", border: "1px solid var(--line)" }}>Remito compartido</button>
              </div>

              <div className="grid gap-2" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(120px,1fr))" }}>
                <div><label style={{ fontSize: 11, color: "#8A8570" }}>Fecha</label><input className="cc-input" style={{ padding: "8px 10px", fontSize: 13 }} type="date" value={form.fecha} onChange={(e) => set("fecha", e.target.value)} /></div>
                <div><label style={{ fontSize: 11, color: "#8A8570" }}>N° Remito</label><input className="cc-input" style={{ padding: "8px 10px", fontSize: 13 }} value={form.remito} onChange={(e) => set("remito", e.target.value)} /></div>
                {!compartido && (
                  <>
                    <div>
                      <label style={{ fontSize: 11, color: "#8A8570" }}>Campo</label>
                      <select className="cc-input" style={{ padding: "8px 10px", fontSize: 13 }} value={form.campoId} onChange={(e) => elegirCampo(e.target.value)}>
                        <option value="">Elegir...</option>
                        {camposConLotes.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                        {lotesDelTipo.some((l) => !l.campoId) && <option value="__sin_campo__">Sin campo asignado</option>}
                      </select>
                    </div>
                    <div>
                      <label style={{ fontSize: 11, color: "#8A8570" }}>Lote</label>
                      <select className="cc-input" style={{ padding: "8px 10px", fontSize: 13 }} value={form.loteId} onChange={(e) => set("loteId", e.target.value)} disabled={!form.campoId}>
                        <option value="">{form.campoId ? "Elegir..." : "Elegí un campo primero"}</option>
                        {(form.campoId === "__sin_campo__" ? lotesDelTipo.filter((l) => !l.campoId) : lotesDelCampo).map((l) => <option key={l.id} value={l.id}>{l.nombre}{l.hectareas ? ` (${fmt(l.hectareas, 1)} ha)` : ""}</option>)}
                      </select>
                    </div>
                  </>
                )}
                <div><label style={{ fontSize: 11, color: "#8A8570" }}>Destino</label><input className="cc-input" style={{ padding: "8px 10px", fontSize: 13 }} value={form.destino} onChange={(e) => set("destino", e.target.value)} placeholder="Silo / planta" /></div>
                <div><label style={{ fontSize: 11, color: "#8A8570" }}>Kg tolva</label><input className="cc-input" style={{ padding: "8px 10px", fontSize: 13 }} type="number" value={form.kgTolva} onChange={(e) => set("kgTolva", e.target.value)} /></div>
                <div><label style={{ fontSize: 11, color: "var(--rust)" }}>Kg brutos</label><input className="cc-input" style={{ padding: "8px 10px", fontSize: 13, borderColor: !form.kgBrutos ? "var(--rust)" : undefined }} type="number" value={form.kgBrutos} onChange={(e) => set("kgBrutos", e.target.value)} /></div>
                <div><label style={{ fontSize: 11, color: "var(--rust)" }}>Kg SL</label><input className="cc-input" style={{ padding: "8px 10px", fontSize: 13, borderColor: !form.kgSL ? "var(--rust)" : undefined }} type="number" value={form.kgSL} onChange={(e) => set("kgSL", e.target.value)} /></div>
                <div><label style={{ fontSize: 11, color: "var(--rust)" }}>Humedad %</label><input className="cc-input" style={{ padding: "8px 10px", fontSize: 13, borderColor: !form.humedad ? "var(--rust)" : undefined }} type="number" value={form.humedad} onChange={(e) => set("humedad", e.target.value)} /></div>
              </div>
              <div style={{ fontSize: 11, color: "#8A8570" }}>Podés guardar con solo fecha, N° y lote — Kg brutos, Kg SL y Humedad los completás cuando tengas los datos de planta (quedan en rojo mientras falten).</div>

              {compartido && (
                <div>
                  <label style={{ fontSize: 11, color: "#8A8570" }}>¿De qué lotes sale este remito?</label>
                  <div className="space-y-2 mt-1">
                    {gruposCampoLote.map((campoId) => {
                      const lotesDeEsteGrupo = campoId === "__sin_campo__" ? lotesDelTipo.filter((l) => !l.campoId) : lotesDelTipo.filter((l) => l.campoId === campoId);
                      if (!lotesDeEsteGrupo.length) return null;
                      return (
                        <div key={campoId}>
                          <div style={{ fontSize: 10.5, fontWeight: 700, color: "#8A8570", textTransform: "uppercase", letterSpacing: ".03em", marginBottom: 3 }}>{nombreDeCampo(campoId)}</div>
                          <div className="flex flex-wrap gap-1">
                            {lotesDeEsteGrupo.map((l) => {
                              const sel = loteIdsCompartido.includes(l.id);
                              return (
                                <button key={l.id} type="button" onClick={() => toggleLoteCompartido(l.id)} className="cc-btn" style={{ padding: "5px 8px", fontSize: 11.5, background: sel ? "var(--soil)" : "#fff", color: sel ? "#fff" : "var(--ink)", border: "1px solid var(--line)" }}>
                                  {sel ? <CheckCircle2 size={12} /> : <MapPin size={12} />} {l.nombre}
                                </button>
                              );
                            })}
                          </div>
              ...
              <div className="flex items-center gap-2">
                <button className="cc-btn cc-btn-primary" style={{ padding: "7px 12px", fontSize: 12.5 }} onClick={guardar}>{editId ? <Pencil size={14} /> : <Plus size={14} />} {editId ? "Guardar cambios" : "Guardar remito"}</button>
                <button className="cc-btn cc-btn-ghost" style={{ padding: "7px 12px", fontSize: 12.5 }} onClick={() => { limpiarForm(); setMostrandoForm(false); }}>Cancelar</button>
                {mensaje && <span style={{ color: "var(--soil-light)", fontWeight: 700, fontSize: 12.5 }}>{mensaje}</span>}
              </div>
            </div>
          )}

          {remitos.length === 0 ? (
            <div style={{ fontSize: 12.5, color: "#8A8570" }}>Todavía no hay remitos cargados para "{tipoCultivo}" en esta campaña.</div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              {remitos.some(faltaValidar) && (
                <div style={{ fontSize: 11.5, color: "var(--rust)", marginBottom: 6, display: "flex", alignItems: "center", gap: 4 }}><AlertCircle size={13} /> Hay remitos con datos pendientes de confirmar (en rojo).</div>
              )}
              <table className="w-full" style={{ fontSize: 12, minWidth: 560 }}>
                <thead><tr style={{ background: "#EEEADA", textAlign: "left" }}><th className="px-2 py-1.5">Fecha</th><th className="px-2 py-1.5">Remito</th><th className="px-2 py-1.5">Lote</th><th className="px-2 py-1.5 text-right">Kg SL</th><th className="px-2 py-1.5 text-right">Hum.%</th><th className="px-2 py-1.5"></th></tr></thead>
                <tbody>
                  {[...remitos].sort((a, b) => (b.fecha || "").localeCompare(a.fecha || "")).map((r) => (
                    <tr key={r.id} style={{ borderTop: "1px solid var(--line)", background: editId === r.id ? "#FDF3E0" : "transparent" }}>
                      <td className="px-2 py-1.5 cc-mono">{r.fecha}</td>
                      <td className="px-2 py-1.5">{r.remito}</td>
                      <td className="px-2 py-1.5">{r.lote || "-"}{r.compartido && <span className="cc-chip" style={{ background: "#EDE7F6", color: "#5B4B8A", marginLeft: 4, fontSize: 10 }}>Compartido</span>}</td>
                      <td className="px-2 py-1.5 text-right cc-mono" style={{ color: !r.kgSL ? "var(--rust)" : "var(--ink)", fontWeight: !r.kgSL ? 700 : 400 }}>{r.kgSL ? fmt(r.kgSL) : "A confirmar"}</td>
                      <td className="px-2 py-1.5 text-right cc-mono" style={{ color: !r.humedad ? "var(--rust)" : "var(--ink)", fontWeight: !r.humedad ? 700 : 400 }}>{r.humedad ? fmt(r.humedad, 1) : "A confirmar"}</td>
                      <td className="px-2 py-1.5 text-right">
                        {puedeEditar && (
                          <div className="flex gap-1 justify-end">
                            <button onClick={() => editar(r)} title="Editar"><Pencil size={14} color="var(--frost)" /></button>
                            <button onClick={() => eliminar(r.id)} title="Eliminar"><Trash2 size={14} color="var(--rust)" /></button>
                          </div>
                        )}
                      </td>
                    </tr>
...
function CosechaVentaCard({ grupo, ventasPool, ventasPoolApi, ventasApiGlobal, puedeEditar }) {
  const [mostrandoForm, setMostrandoForm] = useState(false);
  const [fecha, setFecha] = useState("");
  const [origen, setOrigen] = useState("");
  ...
  const pct = grupo.cosechadoTon ? Math.min((grupo.vendidoTon / grupo.cosechadoTon) * 100, 100) : 0;
  const colorRestante = grupo.restanteTon < -0.01 ? "var(--rust)" : "var(--soil-light)";

  const registrarVenta = async () => {
    const ton = Number(toneladas), pre = Number(precio);
    if (!fecha || !ton || !pre) { alert("Completá fecha, toneladas y U$S/ton."); return; }
    if (ton <= 0 || pre <= 0) { alert("Toneladas y precio tienen que ser mayores a 0."); return; }
    const cultivosConSuperficie = grupo.cultivos.filter((c) => c.superficie > 0);
    if (!cultivosConSuperficie.length) { alert("Ninguno de estos cultivos tiene hectáreas cargadas todavía — andá a 'Lotes' en cada cultivo antes de repartir una venta."); return; }
    setGuardando(true);
    try {
      const superficieTotal = cultivosConSuperficie.reduce((s, c) => s + c.superficie, 0);
      const refs = await Promise.all(cultivosConSuperficie.map((c) => {
        const toneladasCultivo = ton * (c.superficie / superficieTotal);
        return ventasApiGlobal.add({ cultivoId: c.id, fecha, origen, toneladas: toneladasCultivo, dolaresPorTonelada: pre });
      }));
      await ventasPoolApi.add({
        campaniaId: grupo.campaniaId, tipoCultivo: grupo.tipoCultivo, categoria: grupo.categoria,
        fecha, origen, toneladas: ton, dolaresPorTonelada: pre, ventaIds: refs.map((r) => r.id),
      });
      setFecha(""); setOrigen(""); setToneladas(""); setPrecio(""); setMostrandoForm(false);
      setMensaje("Venta repartida ✓"); setTimeout(() => setMensaje(""), 2500);
    } finally { setGuardando(false); }
  };

  const eliminarVenta = async (pool) => {
    if (!confirm("Esto va a borrar la venta y los ingresos que se repartieron en cada cultivo. ¿Continuar?")) return;
    await Promise.all((pool.ventaIds || []).map((id) => ventasApiGlobal.remove(id)));
    await ventasPoolApi.remove(pool.id);
  };
  ...
  return (
    <div className="cc-card p-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          <div className="cc-h" style={{ fontSize: 16, fontWeight: 600 }}>
            <span className="cc-chip" style={{ background: CAT_COLOR[grupo.categoria] + "22", color: CAT_COLOR[grupo.categoria], marginRight: 6 }}>{grupo.categoria === "verano" ? "V" : "I"}</span>
            {grupo.tipoCultivo}
          </div>
          <div style={{ fontSize: 12, color: "#8A8570" }}>{grupo.campaniaNombre} · {grupo.cultivos.length} cultivo{grupo.cultivos.length !== 1 ? "s" : ""} · {fmt(grupo.superficieTotal, 1)} ha</div>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-2 mb-2">
        <MiniStat label="Cosechado" value={`${fmt(grupo.cosechadoTon, 2)} tn`} />
        <MiniStat label="Vendido" value={`${fmt(grupo.vendidoTon, 2)} tn`} />
        <MiniStat label="Resta vender" value={`${fmt(grupo.restanteTon, 2)} tn`} />
      </div>
      <div style={{ background: "#EEEADA", borderRadius: 99, height: 8, marginBottom: 10, overflow: "hidden" }}>
        <div style={{ width: `${pct}%`, background: colorRestante, height: "100%", borderRadius: 99 }} />
      </div>
      {grupo.restanteTon < -0.01 && <div style={{ fontSize: 11.5, color: "var(--rust)", marginBottom: 8 }}>Vendiste más de lo cosechado hasta ahora — revisá los números cuando tengas todos los remitos cargados.</div>}

      {ventasPool.length > 0 && (
        <div className="space-y-1 mb-3" style={{ fontSize: 12 }}>
          {[...ventasPool].sort((a, b) => (b.fecha || "").localeCompare(a.fecha || "")).map((v) => (
            <div key={v.id} className="flex items-center justify-between" style={{ borderTop: "1px solid var(--line)", paddingTop: 4 }}>
              <span>{v.fecha} · {v.origen || "sin origen"} · {fmt(v.toneladas, 2)} tn @ {fmtUSD(v.dolaresPorTonelada)}</span>
              {puedeEditar && <button onClick={() => eliminarVenta(v)}><Trash2 size={14} color="var(--rust)" /></button>}
            </div>
          ))}
        </div>
      )}
        ...
        mostrandoForm ? (
          <div className="space-y-2" style={{ borderTop: "1px solid var(--line)", paddingTop: 10 }}>
            <div className="grid gap-2" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(110px,1fr))" }}>
              <input className="cc-input" type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} />
              <input className="cc-input" value={origen} onChange={(e) => setOrigen(e.target.value)} placeholder="Origen" />
              <input className="cc-input" type="number" value={toneladas} onChange={(e) => setToneladas(e.target.value)} placeholder="Toneladas" />
              <input className="cc-input" type="number" value={precio} onChange={(e) => setPrecio(e.target.value)} placeholder="U$S/ton" />
            </div>
            <div className="flex items-center gap-2">
              <button className="cc-btn cc-btn-primary" style={{ padding: "7px 12px", fontSize: 12.5 }} onClick={registrarVenta} disabled={guardando}>
                {guardando ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />} Registrar venta
              </button>
              <button className="cc-btn cc-btn-ghost" style={{ padding: "7px 12px", fontSize: 12.5 }} onClick={() => setMostrandoForm(false)}>Cancelar</button>
              {mensaje && <span style={{ color: "var(--soil-light)", fontWeight: 700, fontSize: 12.5 }}>{mensaje}</span>}
            </div>
          </div>
        ) : (
          <button className="cc-btn cc-btn-ghost" onClick={() => setMostrandoForm(true)}><Plus size={16} /> Cargar venta</button>
        )
      )}
    </div>
  );
...
function HistorialView({ campos, lotes, cultivos, campanias, remitosTodos, onOpenCultivo }) {
  ...
  // Rendimiento y superficie de cada cultivo (los remitos se cargan por lote, así que el
  // rendimiento de cada cultivo se arma sumando lo que le toca a cada uno de sus lotes)
  const datosCultivo = {};
  cultivos.forEach((c) => {
    const campania = campanias.find((cp) => cp.id === c.campaniaId);
    const superficie = lotes.filter((l) => (c.loteIds || []).includes(l.id)).reduce((s, l) => s + Number(l.hectareas || 0), 0);
    const remitosDeLaCampania = remitosTodos.filter((r) => r.campaniaId === c.campaniaId);
    const totalTon = (c.loteIds || []).reduce((s, loteId) => s + kgAtribuidoALote(remitosDeLaCampania, loteId, lotes), 0) / 1000;
    datosCultivo[c.id] = {
  ...
  const historialDeLote = (loteId) =>
    cultivos.filter((c) => (c.loteIds || []).includes(loteId))
      .map((c) => {
        const remitosDeLaCampania = remitosTodos.filter((r) => r.campaniaId === c.campaniaId);
        const kgLote = kgAtribuidoALote(remitosDeLaCampania, loteId, lotes);
